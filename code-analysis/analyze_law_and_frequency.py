...
# ensure output directory exists for text and figures
os.makedirs('./figures', exist_ok=True)


LANGUAGE_MAP = {}
try:
    # Prefer prebuilt grammars if available (no local build required)
    ...
    logger.info('Loaded languages from tree_sitter_languages')
except Exception as e:
    logger.warning(f'Failed to load prebuilt languages: {e}')
    grammar_paths = [
    ...
    if all(os.path.exists(p) for p in grammar_paths):
        if not os.path.exists('build/my-languages.so'):
            Language.build_library('build/my-languages.so', grammar_paths)
        else:
        ...
        raise FileNotFoundError(
            'Tree-sitter language sources not found. Install prebuilt languages with\n'
            '  pip install tree_sitter_languages\n'
            'or provide grammar repos under ./tree-sitter/ to build locally.'
        )
...
# 定义 token 类别映射（按 Tree-sitter 节点类型归类
token_categories = {
...
# 用 Tree-sitter 得到叶子节点，映射到类别；输出 (token, start, end, category) 列表。
...
if __name__ == '__main__':
    ...
    code_pairs = load_data(dataset='CodeSearchNet', key='Seed-Coder-8B-Instruct-100000-tp0.2', max_num=max_num)
