...
# pip install protobuf sentencepiece prettytable
def load_base_model_and_tokenizer(args, model_config):
    ...
    if '13b' in name:
        ...
        # Seed-Coder models are Llama-compatible; bypass AutoConfig auto-detection
        config = transformers.LlamaConfig.from_pretrained(name, cache_dir=model_config['cache_dir'], trust_remote_code=True)
    ...
    # Decoder-only architectures prefer left padding for generation
    decoder_only = any(x in name.lower() for x in ['llama', 'qwen', 'deepseek', 'gpt', 'opencoder', 'mistral', 'yi', 'open-llama', 'seed'])
    if decoder_only:
        optional_tok_kwargs['padding_side'] = 'left'
    elif args.dataset in ['pubmed']:
        optional_tok_kwargs['padding_side'] = 'left'
    if 'seed' in name.lower():
        # Seed checkpoints often ship a fast tokenizer; prefer AutoTokenizer to avoid SPM/protobuf issues
        base_tokenizer = transformers.AutoTokenizer.from_pretrained(name, **optional_tok_kwargs, cache_dir=model_config['cache_dir'], trust_remote_code=True, use_fast=True)
    elif 'llama' in name.lower():
        base_tokenizer = transformers.LlamaTokenizer.from_pretrained(name, **optional_tok_kwargs, cache_dir=model_config['cache_dir'])
    elif 'opencoder' in name.lower():
        base_tokenizer = transformers.AutoTokenizer.from_pretrained(name, **optional_tok_kwargs, cache_dir=model_config['cache_dir'], trust_remote_code=True)
    else:
