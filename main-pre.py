...
def perturb_texts_(texts, args,  model_config, ceil_pct=False):
    ...
    if args.perturb_type == 'random':
        ...
        perturbed_texts_part2 = perturbed_texts_part2[:n2]
        return perturbed_texts_part1 + perturbed_texts_part2
    elif args.perturb_type == 'random-deadcode-if-false':
        perturbed_texts = [random_insert_deadcode_if_false(
            x, pct) for x in texts]
        return perturbed_texts
    elif args.perturb_type == 'random-deadcode-unused-var':
        perturbed_texts = [random_insert_deadcode_unused_var(
            x, pct) for x in texts]
        return perturbed_texts
    elif args.perturb_type == 'random-deadcode-redundant-ops':
        perturbed_texts = [random_insert_deadcode_redundant_ops(
            x, pct) for x in texts]
        return perturbed_texts
    else:
...
def random_insert_deadcode_if_false(text, pct=0.3):
    lines = text.split('\n')
    n_lines = len(lines)
    if n_lines == 0:
        return text
...
def random_insert_deadcode_unused_var(text, pct=0.3):
    lines = text.split('\n')
    n_lines = len(lines)
    if n_lines == 0:
        return text
    ...
    for idx in sorted(idxs, reverse=True):
        ref_line = lines[idx-1] if idx-1 >= 0 else (lines[0] if n_lines > 0 else '')
        indent = _leading_indent(ref_line)
        val = random.choice(['0', '1', 'None', '[]', '{}', "''"])  # noqa: E501
        var = f"unused_{random.randint(0, 10**9)}"
        lines.insert(idx, f"{indent}{var} = {val}")
    return '\n'.join(lines)


def random_insert_deadcode_redundant_ops(text, pct=0.3):
    lines = text.split('\n')
    n_lines = len(lines)
    if n_lines == 0:
        return text
...
def main():
    ...
    if args.perturb_type == 'random':
        ...
        masked_texts = [random_insert_deadcode_if_false(
            x, pct) for x in texts]
    elif args.perturb_type == 'random-deadcode-unused-var':
        masked_texts = [random_insert_deadcode_unused_var(
            x, pct) for x in texts]
    elif args.perturb_type == 'random-deadcode-redundant-ops':
        masked_texts = [random_insert_deadcode_redundant_ops(
            x, pct) for x in texts]
    else:
        raise ValueError(f'Unknown perturb_type: {args.perturb_type}')
    ...
    perturb_fn = functools.partial(
        perturb_texts, args=args, model_config=model_config)  # perturbation function
    p_sampled_text = perturb_fn([x for x in sampled_text for _ in range(
        max(n_perturbation_list))])  # perturb sampled text
    p_original_text = perturb_fn([x for x in original_text for _ in range(
        max(n_perturbation_list))])  # perturb original text
