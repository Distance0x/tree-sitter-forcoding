...
def load_data(path='data/CodeSearchNet', language='python', max_num=10000):
    ...
    if 'humaneval' in path:
        ...
        # 指定数据文件路径
        data_dir = 'data/TheVault'
        parquet_files = [
            os.path.join(data_dir, 'python-00000-of-00002.parquet'),
            os.path.join(data_dir, 'python-00001-of-00002.parquet')
        ]
        ...
        for file in parquet_files:
            if os.path.exists(file):
                ...
                logger.info(f'Loaded {len(df_part)} samples from {file}')
            else:
        ...
        logger.info(f'Total loaded {len(df)} samples from TheVault dataset')
        ...
        for _, row in tqdm(df.iterrows(), total=len(df)):
            # 尝试不同的字段名
            original_string = row.get('func_code_string', row.get('original_string', ''))
...
def generate_hf(model_name, prompts, solutions, batch_size=16, max_length_sample=128, max_length=128, do_sample=True, top_p=0.95, temperature=0.2):
    ...
    if 'starcoder' in model_name.lower() or "llama" in model_name.lower() or "wizard" in model_name.lower() or "codegen2" in model_name.lower():
        ...
        for input_ids in tqdm(input_ids, ncols=50):
            ...
            # 避免设置冲突 优先max_length = 256 使得生成变快
            if max_length_sample >= 256:
                outputs = model.generate(
                    input_ids, 
                    do_sample=do_sample, 
                    max_new_tokens=max_length_sample,  # 使用max_new_tokens替代max_length
                    top_p=top_p, 
        ...
        for input_ids in tqdm(dataloader, ncols=50):
            ...
            outputs = model.generate(
                input_ids, 
                do_sample=do_sample, 
                max_new_tokens=max_length_sample,  # 使用max_new_tokens
                top_p=top_p, 
                temperature=temperature, 
                pad_token_id=tokenizer.pad_token_id, 
                use_cache=True
